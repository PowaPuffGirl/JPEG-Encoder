...
public:
    ...
    void transformBlock(const std::function<const vec8 &(uint)>& get, const std::function<T &(uint, uint)>& set) {
        for (uint i = 0; i < 8; i++) {
            const auto& row = get(i);
            y0[i] = row[0];
            y1[i] = row[1];
            y2[i] = row[2];
            y3[i] = row[3];
    ...
    void arai(const std::function<T &(uint, uint)>& set) {
