...
struct SOS {
    ...
    uint16_t len = convert_u16(12); // length of the segment 6+ 2* component amount
    uint8_t componentAmount = 0x03; // amount of components that are used in the image >= 1 and <= 4
    ...
    uint16_t  yht = convert_u16(0b0000000100010000);
    uint16_t cbht = convert_u16(0b0000001000110010);
    uint16_t crht = convert_u16(0b0000001100110010);
