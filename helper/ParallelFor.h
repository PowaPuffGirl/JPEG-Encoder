...
private:
    ...
    class ParallelStateTask : public Task {
    public:
        ParallelStateTask(std::promise<void> &awake, const int min, const int max, const int threadNr,
                const std::function<void(const int, const int, const int)> &fn) :
                Task(awake, min, max), threadNr(threadNr), fn(fn) {}

        void Run() {
            fn(Task::min, Task::max, threadNr);
        }
...
public:
    ...
    void RunP(const std::function<void(const int, const int, const int)>& fn, const int min, const int max) {
        const int range = max - min;
        const int singleRange = range/threads;
        ...
        if(singleRange < singleThreadLimit) {
            ...
            std::unique_lock<std::mutex> lck(lock);
            for (int i = 0; i < threads; ++i) {
                tasks[i].push(new ParallelStateTask(promises[i], min + (i * singleRange),
                                   (i == threads - 1) ? max : min + (i + 1) * singleRange, i, fn
                ));
            }
            lck.unlock();
            notifier.notify_all();
